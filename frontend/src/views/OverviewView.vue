...
<template>
  <section class="page-stack">
    ...
    <div class="metric-grid">
      ...
      <article class="metric card"><AudioLines/><div><small>控制台语音</small><strong>{{ store.state.voice?.speechConfigured?'可对话':store.state.voice?.asrConfigured?'可识别':'待配置' }}</strong><span>{{ store.state.voice?.transcribing?'正在识别':store.state.voice?.synthesizing?'正在生成语音':'麦克风 · 扬声器' }}</span></div></article>
    </div>
