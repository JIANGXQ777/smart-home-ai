...
<template>
  <form class="page-stack model-config-page" @submit.prevent="save">
    ...
    <div v-else class="model-config-grid">
      ...
      <section class="card model-card">
        <div class="model-card-head"><div class="model-icon asr"><AudioLines :size="22"/></div><div><small>AUTOMATIC SPEECH RECOGNITION</small><h2>ASR 语音识别</h2><p>把麦克风采集的声音转换为文字</p></div><label class="model-switch"><span class="sr-only">启用 ASR</span><input v-model="models.asr.enabled" type="checkbox"/></label></div>
